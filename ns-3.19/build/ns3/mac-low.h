...
public:
   ...
   */
  virtual void GotBlockAck (const CtrlBAckResponseHeader *blockAck, Mac48Address source, WifiMode mode);
  /**
...
public:
  ...
  /**
   * \param packet the packet to be aggregated
   ...
   * \param hdr the WifiMacHeader for the packet.
   *
   * This function creates the actual A-MPDU from the packets in the aggregate queue
   * 
   */
  Ptr<Packet> CreateAmpdu (Ptr<const Packet> packet, const WifiMacHeader hdr);
  /**
...
private:
  ...
  void SendPacket(Ptr<const Packet> packet, WifiPreamble preamble, WifiTxVector 
      txVector,uint8_t packetType);
