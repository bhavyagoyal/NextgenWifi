...
public:
   ...
   * \param packetType the type of the packet 0 is not A-MPDU, 1 is a MPDU that 
      is part of an A-MPDU  and 2 is the last MPDU in an A-MPDU
   * \param incFlag this flag is used to indicate that the static variables need
       to be updated or not. This function is called a couple of times for the same packet so static variables should not be increased each time
   * \return the total amount of time this PHY will stay busy for
   *          the transmission of these bytes.
   */
  static Time CalculateTxDuration (uint32_t size, WifiTxVector txvector, enum WifiPreamble preamble,uint8_t packetType,uint8_t incFlag);
...
/** 
   ...
   */
  static double GetPayloadDurationMicroSeconds (uint32_t size, WifiTxVector txvector,WifiPreamble preamble,uint8_t packetType,uint8_t incFlag);
  ...
  */
   ...
   */
  virtual void SetChannelNumber (uint64_t id) = 0;
  /**  
   * Return current channel number.
   *
   * \return the current channel number
   */
  virtual uint64_t GetChannelNumber () const = 0;
  ...
  static WifiMode GetOfdmRate29_3MbpsBW80MHz ();
  static WifiMode GetOfdmRate58_5MbpsBW80MHz ();
  static WifiMode GetOfdmRate87_8MbpsBW80MHz ();
  static WifiMode GetOfdmRate117MbpsBW80MHz ();
  static WifiMode GetOfdmRate175_5MbpsBW80MHz ();
  static WifiMode GetOfdmRate234MbpsBW80MHz ();
  static WifiMode GetOfdmRate263_3MbpsBW80MHz ();
  static WifiMode GetOfdmRate292_5MbpsBW80MHz ();
   ...
   */
  void NotifyMonitorSniffRx (Ptr<const Packet> packet, uint16_t channelFreqMhz, uint64_t channelNumber, uint32_t rate, bool isShortPreamble,
   ...
   */
  void NotifyMonitorSniffTx (Ptr<const Packet> packet, uint16_t channelFreqMhz, uint64_t channelNumber, uint32_t rate, bool isShortPreamble, uint8_t txPower);
...
private:
  ...
  // toal number of symbols sent from an A-MPDU so far
  static uint32_t m_totalApmduNumSymbols;
  //total size of sent MPDUs that are part of an A-MPDU
  static uint32_t m_totalAmpduSize;
