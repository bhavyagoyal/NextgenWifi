...
private:
   ...
   */
  Time GetEnergyDuration (double energyW, uint64_t primary);
  Time GetEnergyDurationFromT (double energyW, uint64_t pos_channel, Time now);
   ...
   */
  struct InterferenceHelper::SnrPer CalculateSnrPer (Ptr<InterferenceHelper::Event> event, bool useLUT, uint32_t coding);
  /**
  ...
  double GetPERPreamble (Ptr<InterferenceHelper::Event> event, uint32_t coding);
private:
  /**
...
private:
  ...
  /**
   * typedef for a vector of NiChanges lis for each channel
   */
  typedef std::vector <NiChanges> NiChanges_list;
  /**
  ...
  double CalculatePer_LUT (Ptr<const Event> event, NiChanges *ni, uint32_t coding) const;
  
  uint32_t WifiModeToMcs (WifiMode mode) const;
  double getPer(uint32_t coding, uint32_t PL0, uint32_t chunk_mcs, double snr) const;

  double m_noiseFigure; /**< noise figure (linear) */
  Ptr<ErrorRateModel> m_errorRateModel;
  /// Experimental: needed for energy duration calculation
  NiChanges_list m_niChangesList;
  NiChanges m_niChanges;
  //uint64_t m_activeChannels;
  std::vector<double> m_firstPowers;
  bool m_rxing;
