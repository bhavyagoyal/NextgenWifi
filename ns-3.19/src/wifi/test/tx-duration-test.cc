...
TxDurationTest::CheckPayloadDuration (uint32_t size, WifiMode payloadMode, uint32_t knownDurationMicroSeconds)
{
  ...
  uint32_t calculatedDurationMicroSeconds = WifiPhy::GetPayloadDurationMicroSeconds (size, txVector,WIFI_PREAMBLE_LONG,0,0);
...
TxDurationTest::CheckTxDuration (uint32_t size, WifiMode payloadMode, WifiPreamble preamble, double knownDurationMicroSeconds)
{
  ...
  txVector.SetStbc(0);
  txVector.SetNess(0);
  double calculatedDurationMicroSeconds = WifiPhy::CalculateTxDuration (size, txVector, preamble,0,0).GetMicroSeconds ();
  if (calculatedDurationMicroSeconds != knownDurationMicroSeconds)
    {
      std::cerr << " size=" << size
                << " mode=" << payloadMode
