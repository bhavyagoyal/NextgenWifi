...
public:
   ...
   */
  void SetChannelNumber (uint64_t id);
  /**
   * Set the active channel number.
   *
   * \param id the channel number
   */
  void SetActiveChannels (uint64_t id);
   ...
   * \param id the channel number
   */
  void SetPrimaryChannel (uint64_t id);
  /**
   * Return the current channel number.
   *
   * \return the current channel number
   */
  uint64_t GetChannelNumber () const;
   ...
   * \return the number of channels
   */
  uint64_t GetNumCh_active () const;
  /**
   * Return the number of channels.
   *
   * \return the number of channels
   */
  uint64_t GetNumCh_all () const;
  /**
   * Return the posotion of primary channel.
   *
   * \return the position of primary channel
   */
  uint64_t GetPosPrimary () const;
  uint64_t GetActiveChannelsPos () const;
  /**
   * Return the active channel number.
   *
   * \return the active channel number
   */
  uint64_t GetActiveChannels () const;
  /**
   * Return the primary channel number.
   *
   * \return the primary channel number
   */
  uint64_t GetPrimaryChannel () const;
  /**
   ...
   * \param packetType The type of the received packet (values: 0 not an A-MPDU, 1 is all packets in an A-MPDU except last, 2 is last packet in an A-MPDU)
   */
  ...
  */
  ...
  /**
   * \param use whether to use new interference model
   */
  virtual void SetUseNewModel (bool use);
  /**
   * \param use whether to use MAC header examination at phy layer
   */
  virtual void SetCheckAddrAtPhy (bool use);
  /**
   * \param use whether to use LUT for PER estimation
   */
  virtual void SetLUTforPER (bool use);
  /**
   * \return the choice of interference model usage
   */
  virtual bool GetUseNewModel (void) const;
  /**
   * \return the choice of MAC header examination at phy
   */
  virtual bool GetCheckAddrAtPhy (void) const;
  /**
   * \return the choice of using LUT for PER
   */
  virtual bool GetLUTforPER (void) const;
  /**
   * \param bw the Bandwidth on this node (20 MHz or 40 MHz or 80 MHz).
   */
  virtual void SetBW (uint32_t bw);
  /**
   * \param nt - 1 for STA, 2 for AP
   */
  virtual void SetNodeType (uint32_t nt);
  /**
   * \param nt - 0 for BCC, 1 for LDPC
   */
  virtual void SetCodingType (uint32_t nt);
  /**
   * \return the Bandwidth on this node
   */
  virtual uint32_t GetBW (void) const;
  /**
   * \return the type (STA(1) or AP(2)) of this node
   */
  virtual uint32_t GetNodeType (void) const;
  /**
   * \return the coding type (BCC(0) or LDPC(1)) of this node
   */
  virtual uint32_t GetCodingType (void) const;
  /**
...
private:
  ...
  uint64_t             m_channelNumber;  //!< Operating channel number
  uint64_t             m_activeChannels;  //!< Active channel numbers
  uint64_t             m_primaryChannel;  //!< Primary channel number
  ...
  double m_BW;							//!< Channel bandwidth
  uint32_t m_nt;						//!< 1 for STA, 2 for AP
  uint32_t m_coding;                    //!< 0 for BCC, 1 for LDPC
  bool m_newModel;						//!< Use new Interference Model or not
  bool m_addrCheck;						//!< Use MAC header examination at phy or not
  bool m_useLUT;							//!< USe LUT for PER estimation or not
  Ptr<WifiPhyStateHelper> m_state;      //!< Pointer to WifiPhyStateHelper
  //InterferenceHelper m_interference;    //!< Pointer to InterferenceHelper
  Time m_channelSwitchDelay;            //!< Time required to switch between channel
  double m_divide;
  uint16_t m_mpdusNum;                  //!< carries the number of expected mpdus that are part of an A-MPDU
  uint8_t m_firstMpduStatus;            //!< provides information about whether the first MPDU in an A-MPDU is received correctly or not
