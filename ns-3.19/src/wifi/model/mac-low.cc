...
Ptr<const Packet> 
MacLowBlockAckEventListener::PeekNextPacketInBaQueue (WifiMacHeader &Hdr,uint8_t tid,Mac48Address recipient)
{
 return 0;
}

void 
MacLowBlockAckEventListener::RemoveFromBaQueue (Ptr<const Packet> packet,uint8_t tid,Mac48Address recipient, uint16_t seqnumber)
{
}
...
MacLow::GetDataTxVector (Ptr<const Packet> packet, const WifiMacHeader *hdr) const
{
  ...
  WifiTxVector wtv = m_stationManager->GetDataTxVector (to, hdr, packet, size);
  Ptr<YansWifiPhy> yans = DynamicCast<YansWifiPhy> (m_phy);
  if(yans->GetDivider () == 2) {
	    WifiMode mode1 = WifiPhy::GetOfdmRate52MbpsBW20MHz ();
	    wtv.SetMode(mode1);
  }
  return wtv;
   ...
   for (;queueSize > 0; queueSize--)
     {
       ...
       if (delay ==Seconds(0))
         ...
         {
           m_mpduTxEnd=Simulator::Schedule (delay, &MacLow::SendPacket, this,newPacket, preamble, txVector,packetType);
         }
...
void
MacLow::SendPacket(Ptr<const Packet> packet, WifiPreamble preamble, WifiTxVector txVector, uint8_t packetType)
{
  NS_LOG_DEBUG("Sending MPDU as part of A-MPDU");
...
MacLow::CtsTimeout (void)
{
  ...
  m_listener = 0;
  //flush the packets in the aggregate queue
             ...
             /* MSDU aggregation happened in edca if the user asked for it so m_cuurentPacket may contains a normal packet or a A-MSDU*/
              ...
              uint16_t currentSequenceNumber=0; 
               ...
               if (!hdr.IsBlockAckReq())
                ...
                Ptr<const Packet> peekedPacket= listenerIt->second->PeekNextPacketInBaQueue(peekedHdr, tid,hdr.GetAddr1());
                if (peekedPacket==0)
                  {
               ...
               while (IsInWindow (currentSequenceNumber,startingSequenceNumber, 63) && listenerIt->second->GetNOutstandingPackets(hdr.GetAddr1(), tid) < 63 &&!StopAggregation (peekedPacket,peekedHdr,currentAggregatedPacket, blockAckSize))
                  {
                   ...
                   if (aggregated)
                    {
                       ...
                       if (retry)
                          listenerIt->second->RemoveFromBaQueue(peekedPacket,tid,hdr.GetAddr1(),peekedHdr.GetSequenceNumber());
                       else
                  ...
                  if (retry==true)
                    {
                      peekedPacket= listenerIt->second->PeekNextPacketInBaQueue(peekedHdr,tid,hdr.GetAddr1());
