...
public:
  ...
  void Send_NewModel (Ptr<YansWifiPhy> sender, Ptr<const Packet> packet, double txPowerDbm,
             WifiTxVector txVector, WifiPreamble preamble, uint8_t packetType);
			 
  double CalculateAttenuation20MHzQuantum2_4Ghz (uint32_t overlap, uint64_t high[], uint32_t indx_high, uint64_t low[], uint32_t indx_low, uint32_t BW);

  double CalculateAttenuation20MHzQuantum5Ghz (uint32_t overlap, uint64_t high[], uint32_t indx_high, uint64_t low[], uint32_t indx_low, uint32_t BW);
