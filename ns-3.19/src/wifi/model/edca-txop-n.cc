...
public:
  ...
  virtual Ptr<const Packet> PeekNextPacketInBaQueue (WifiMacHeader &Hdr,uint8_t tid,Mac48Address recipient)
  {
     return m_txop->PeekNextRetransmitPacket(Hdr,tid,recipient);
  }
  virtual void RemoveFromBaQueue (Ptr<const Packet> packet,uint8_t tid,Mac48Address recipient, uint16_t seqnumber)
  {
     m_txop->RemoveRetransmitPacket(packet,tid,recipient,seqnumber);
  }
...
Ptr<const Packet>
EdcaTxopN::PeekNextRetransmitPacket (WifiMacHeader &Hdr,uint8_t tid,Mac48Address recipient)
{
  return m_baManager->PeekNextPacket (Hdr,tid,recipient);
}

void
EdcaTxopN::RemoveRetransmitPacket (Ptr<const Packet> p,uint8_t tid,Mac48Address recipient, uint16_t seqnumber)
{
   m_baManager->RemovePacket (p,tid,recipient,seqnumber);
}
void
EdcaTxopN::NotifyAccessGranted (void)
{
	//std::cout<<m_queue<<"  "<<temp++<<"\n";
  NS_LOG_FUNCTION (this);
  ...
  else
    {
      ...
      if (NeedFragmentation () && ((m_currentHdr.IsQosData ()
      ...
      else
        {
          ...
          //WifiTxVector dataTxVector = m_low->GetDataTxVector (m_currentPacket, &m_currentHdr);
          Time TXOP = Seconds (0.001504);
          WifiMacHeader m_nextHdr;
          Ptr<const Packet> nextPacket = m_queue->PeekFirstAvailable (&m_nextHdr, m_currentPacketTimestamp, m_qosBlockedDestinations);
          if((m_low->CalculateOverallTxTime(m_currentPacket, &m_currentHdr, params) + m_low->GetSifs ())  < TXOP && nextPacket != 0) {
			params.EnableNextData (nextPacket->GetSize ());
			curr_txop = TXOP - (m_low->CalculateOverallTxTime(m_currentPacket, &m_currentHdr, params) + m_low->GetSifs ());
		  }
		  else
			params.DisableNextData ();
...
EdcaTxopN::StartNext (void)
{
  ...
  if(curr_txop > Seconds(0.0)) { 
	  ...
	  Ptr<const Packet> nextPacket = m_queue->PeekFirstAvailable (&m_nextHdr, m_currentPacketTimestamp, m_qosBlockedDestinations);
          ...
          if((m_low->CalculateOverallTxTime(m_currentPacket, &m_currentHdr, params) + m_low->GetSifs ())  < curr_txop && nextPacket != 0) {
			params.EnableNextData (nextPacket->GetSize ());
		  ...
		  curr_txop = curr_txop - (m_low->CalculateOverallTxTime(m_currentPacket, &m_currentHdr, params) + m_low->GetSifs ());
          m_low->StartTransmission (m_currentPacket, &m_currentHdr,
                                    params, m_transmissionListener);
