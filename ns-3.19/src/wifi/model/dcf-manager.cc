  ...
  for (States::const_iterator i = m_states.begin (); i != m_states.end (); k++)
    {
      ...
      if (state->IsAccessRequested ()
          && GetBackoffEndFor (state) <= Simulator::Now () )
        {
          ...
          uint64_t num_ch = yans->GetNumCh_all ();
          uint64_t primary = yans->GetPosPrimary ();
          bool *isAvailable = new bool[num_ch];
          uint64_t loop;
          for(loop=0;loop<num_ch;loop++) {
			  isAvailable[loop] = false;
		  }
		  isAvailable[primary] = true;
		  for(loop=0;loop<num_ch;loop++) {
			  if(loop!=primary) {
				  Time t = yans->m_interference.GetEnergyDurationFromT (yans->GetCcaMode1Threshold(), loop, Simulator::Now () - Time("16us"));
				  if(t.IsZero ()) {
					  isAvailable[loop] = true;
				  }
			  }
		  }
		  SelectActiveChannels(isAvailable, num_ch);
		  //std::cout<<isAvailable[primary]<<std::endl;
          //Time t = yans->m_interference.GetEnergyDurationFromT (yans->GetCcaMode1Threshold(), 0, Simulator::Now () - Time("16us"));
          //std::cout<<t<<std::endl;
...
void 
DcfManager::SelectActiveChannels(bool *isAvailable, uint64_t size)
{
	Ptr<YansWifiPhy> yans = DynamicCast<YansWifiPhy> (m_phy);
	if (yans->GetBW() == 20) {
		yans->SetActiveChannels (yans->GetPrimaryChannel ());
		yans->SetDivider(1.0);
	}
	else if(yans->GetBW () == 40) {//std::cout<<size<<"  "<<yans->GetPosPrimary ()<<std::endl;
		uint64_t loop;
		for(loop=0; loop<size; loop++) {
			if(loop != yans->GetPosPrimary () && isAvailable[loop] == true){
				yans->SetActiveChannels (yans->GetChannelNumber ());
				yans->SetDivider(1.0);
			}
			else {
				yans->SetActiveChannels (yans->GetPrimaryChannel ());
