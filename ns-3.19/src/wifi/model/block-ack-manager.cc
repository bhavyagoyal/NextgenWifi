...
Ptr<const Packet>
BlockAckManager::PeekNextPacket (WifiMacHeader &hdr,uint8_t tid,Mac48Address recipient)
{
   NS_LOG_FUNCTION (this);
  Ptr<const Packet> packet = 0;
  CleanupBuffers ();
  AgreementsI agreement = m_agreements.find (std::make_pair (recipient, tid));
  NS_ASSERT (agreement != m_agreements.end());
  //64 because this is the number used in ns3 should be changed to bufferSize
  std::list<PacketQueueI>::iterator it = m_retryPackets.begin ();    
...
bool
BlockAckManager::RemovePacket (Ptr<const Packet> packet,uint8_t tid,Mac48Address recipient, uint16_t seqnumber)
{
      ...
      if ((*it)->hdr.GetAddr1 () == recipient && (*it)->hdr.GetQosTid () == tid && (*it)->hdr.GetSequenceNumber () == seqnumber)
        {
          ...
          uint8_t tid = hdr.GetQosTid ();
          Mac48Address recipient = hdr.GetAddr1 ();
           if (! ExistsAgreementInState (recipient, tid, OriginatorBlockAckAgreement::ESTABLISHED)) //   && ! SwitchToBlockAckIfNeeded (recipient, tid, hdr.GetSequenceNumber ())
              {
                 /* From section 9.10.3 in IEEE802.11e standard:
                  * In order to improve efficiency, originators using the Block Ack facility
                  * may send MPDU frames with the Ack Policy subfield in QoS control frames
...
void
BlockAckManager::SetWifiRemoteStationManager (Ptr<WifiRemoteStationManager> remoteManager)
{
                  ...
                  if (blockAck->IsFragmentReceived ((*queueIt).hdr.GetSequenceNumber (),
                  ...
                  else
                    {
                      ...
                      if (!AlreadyExists((*queueIt).hdr.GetSequenceNumber (),recipient,tid))
                        {
                          NS_LOG_INFO ("Adding to retry queue " <<(*queueIt).hdr.GetSequenceNumber ()); 
                          m_retryPackets.push_back (queueIt);
                        }
                      queueIt++;
                    }
...
bool
BlockAckManager::NeedBarRetransmission (uint8_t tid,uint16_t seqNumber,Mac48Address recipient)
{
  ...
  CleanupBuffers();
  //64 because this is the number used in ns3 should be changed to bufferSize
  if ((seqNumber+64) < it->second.first.GetStartingSequence())
    return false;
  else
    return true;
}
...
BlockAckManager::GetSeqNumOfNextRetryPacket (Mac48Address recipient, uint8_t tid) const
{
  ...
  NS_FATAL_ERROR ("Couldn't find the sequence number of a packet in the retry queue");
  return 4096;
}
