...
public:
  ...
  /*
  double CalcRxPower_STA_STA (double txPowerDbm,
                      Ptr<MobilityModel> a,
                      Ptr<MobilityModel> b);
  double CalcRxPower_STA_AP (double txPowerDbm,
                      Ptr<MobilityModel> a,
                      Ptr<MobilityModel> b);                                      
  double CalcRxPower_AP_AP (double txPowerDbm,
                      Ptr<MobilityModel> a,
                      Ptr<MobilityModel> b); 
                      */                
  /**
 ...
 * \ingroup propagation
 *
 * \brief a channel B propagation model.
 *
 * This model calculates the reception power with a so-called
 * channel B propagation model:
 * \f$ L = L_0 + 10 n log_{10}(\frac{d}{d_0})\f$
 *
 * where:
 *  - \f$ n \f$ : the path loss distance exponent
 ...
 *
 * When the path loss is requested at a distance smaller than
 * the reference distance, the free space loss model is used.
 *
 */
class ChannelBPropagationLossModel : public PropagationLossModel
{
public:
  static TypeId GetTypeId (void);
  ChannelBPropagationLossModel ();
   ...
   * \param frequency (Hz)
   *
   * Set the carrier frequency used in the Channel B model 
   * calculation.
   */
  void SetFrequency (double frequency);
  ...
  double CalcRxPowerforTest (double txPowerDbm,
                                double distance);
  double shadowing_factor(double mu, double sigma) const;
 
private:
  ChannelBPropagationLossModel (const ChannelBPropagationLossModel &o);
  ChannelBPropagationLossModel & operator = (const ChannelBPropagationLossModel &o);
  virtual double DoCalcRxPower (double txPowerDbm,
                                Ptr<MobilityModel> a,
                                Ptr<MobilityModel> b) const;
  virtual int64_t DoAssignStreams (int64_t stream);
  ...
  double m_referenceDistance;
  double m_referenceLoss;
  ///Shadow fading standard deviation before reference distance
  double m_sfsdbrd;
  ///Shadow fading standard deviation after reference distance
  double m_sfsdard;
  static const double PI;
  double m_lambda;
  double m_frequency;
 ...
 * \ingroup propagation
 *
 * \brief a outdoor propagation model.
 *
 */
class OutdoorPropagationLossModel : public PropagationLossModel
{
public:
  static TypeId GetTypeId (void);
  OutdoorPropagationLossModel ();

  
  void SethBS (double hbs);
  void SethMS (double hms);
  ...
  virtual double DoCalcRxPower_STA_STA (double txPowerDbm,
                                Ptr<MobilityModel> a,
                                Ptr<MobilityModel> b) ;
  virtual double DoCalcRxPower_STA_AP (double txPowerDbm,
                                Ptr<MobilityModel> a,
                                Ptr<MobilityModel> b);
  virtual double DoCalcRxPower_AP_AP (double txPowerDbm,
                                Ptr<MobilityModel> a,
                                Ptr<MobilityModel> b);
 
private:
  OutdoorPropagationLossModel (const OutdoorPropagationLossModel &o);
  OutdoorPropagationLossModel & operator = (const OutdoorPropagationLossModel &o);
  virtual double DoCalcRxPower (double txPowerDbm,
                                Ptr<MobilityModel> a,
                                Ptr<MobilityModel> b) const;
 ...
 * \ingroup propagation
 *
 * \brief a channel D propagation model.
 *
 * This model calculates the reception power with a so-called
 * channel D propagation model:
 * \f$ L = L_0 + 10 n log_{10}(\frac{d}{d_0})\f$
 *
 * where:
 *  - \f$ n \f$ : the path loss distance exponent
 ...
 *
 */
class ChannelDPropagationLossModel : public PropagationLossModel
{
public:
  static TypeId GetTypeId (void);
  ChannelDPropagationLossModel ();
   ...
   * \param frequency (Hz)
   *
   * Set the carrier frequency used in the Channel D model 
   * calculation.
   */
  void SetFrequency (double frequency);
...
private:
  ChannelDPropagationLossModel (const ChannelDPropagationLossModel &o);
  ChannelDPropagationLossModel & operator = (const ChannelDPropagationLossModel &o);
  virtual double DoCalcRxPower (double txPowerDbm,
                                Ptr<MobilityModel> a,
                                Ptr<MobilityModel> b) const;
  virtual int64_t DoAssignStreams (int64_t stream);
